...
**🚧 批次 4.1 — GitHub Code Search（代码片段盲区，进行中）**
- [x] 类型扩展（`WheelSource: 'github-code'`、`WheelType: 'snippet'`、`GitHubCodeRawResult`）
- [x] `githubCodeSourceAdapter` 实现（`/search/code`，强制认证，10 req/min，text-match media type）
- [x] 关键字补齐（实现/函数/代码/片段/示例/源码 → implementation/function/snippet/example/source）
- [x] VS Code Marketplace 适配器（`extensionquery` POST API，IDE 插件盲区）
- [ ] Papers with Code 适配器（算法盲区）
