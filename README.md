...
[![Tests](https://img.shields.io/badge/tests-696%2F696-brightgreen.svg?style=flat-square)](./)
...
| 🌏 | **中文友好** | 260+ 词的中英翻译表（含小红书→xiaohongshu/rednote、s型加减速→s-curve-acceleration、多个平台→multi-platform、主题→theme 等专业术语），中文 query 自动转英文搜索；意图前缀自动剥离（"我想做"/"我要在我的X中增加"等 40+ 模式）；虚词自动剥离（一个/等等/的工具）；翻译幂等性避免重复追加 |
...
> 📖 进阶用法（换词重搜、排除重搜）见 [USAGE.md FAQ](./docs/USAGE.md#faq)
...
| `FINDAWHEEL_LIMIT` | 否 | `50` | 默认返回结果数量。 |
...
<!--
SEO 关键词索引(隐藏注释,不影响阅读,仅供搜索引擎和 AI 索引):

工具类型:
MCP server, MCP 服务, Model Context Protocol, AI 编程助手, 代码搜索工具, 库搜索工具, package discovery tool, library finder, dependency finder

核心功能:
避免重复造轮子, 找轮子, 复用代码, 开源项目搜索, 包搜索, 轮子查找, 依赖发现, avoid reinventing the wheel, find existing wheel, library search, code reuse

使用场景:
AI 编程, 生成式开发, vibe coding, RAG, 检索增强生成, Trae, Cursor, Claude Desktop, Claude Code, Windsurf, Cline, AI pair programming, AI 辅助编程

数据源 / 生态:
GitHub 搜索, npm 搜索, PyPI 搜索, crates.io 搜索, Maven Central, RubyGems, pkg.go.dev, Gitee, GitLab, Libraries.io, GitHub Code Search, VS Code Marketplace, Papers with Code, HuggingFace, Exa, Tavily, 多平台搜索, 15 个数据源

同义词 / 长尾词:
重复造轮子, 轮子查找器, 代码复用工具, 开源发现, wheel finder, library finder, dependency discovery, how to find existing library, AI coding search tool
-->
