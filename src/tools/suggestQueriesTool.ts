...
export function createSuggestQueriesTool() {
  async function handle(input: SuggestQueriesInput): Promise<McpToolResult> {
    ...
    // 翻译不完整检测:query 含未翻译的中文技术词 → 翻译表未覆盖该领域
    // 提示 AI client 自行生成英文 query,避免直接用 translatedQuery 召回差
    if (hasUntranslatedZh(input.query)) {
      output.warning = `⚠️ 翻译不完整预警:query 含中文技术词且翻译表未覆盖,直接搜索会召回差。建议:AI client 自行将用户意图翻译为精准英文 query 后再调 find_wheel(例如"算卦看风水"→"divination feng-shui i-ching fortune-telling")。`;
    }
